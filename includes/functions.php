...
//NEXT: Utility function to count the current enrollment of irreg students for the given course, year, term, SY, and section
   ...
   function loadDropdownValues($grade_values, $current_value){
    for ($i=0; $i < count($grade_values); $i++) { 
      if ($grade_values[$i] == $current_value) {
        echo "<option selected value=\"".$grade_values[$i]."\">".$grade_values[$i]."</option>";
      }
      else{
        echo "<option value=\"".$grade_values[$i]."\">".$grade_values[$i]."</option>";
      }
