...
//loop through values of the input arrays
  foreach ($prelim as $key => $value) {
	...
	if ($result === TRUE) {
      redirect_to("faculty-encode-grades.php?subject_id=".$subject_id."&term=".urlencode($term)."&school_yr=".urlencode($school_yr)."&course_id=".urlencode($course_id)."&year=".urlencode($year)."&section=".urlencode($section)."&teacher_id=".urlencode($row['teacher_id'])."&grade_posted=1");
...
//loop through values of the input arrays
  ...
  if ($result === TRUE) {
         ...
         $URL= "faculty-encode-grades.php?subject_id=".urlencode($subject_id)."&term=".urlencode($term)."&school_yr=".urlencode($school_yr)."&course_id=".urlencode($course_id)."&year=".urlencode($year)."&section=".urlencode($section)."&teacher_id=".urlencode($teacher_id)."&grade_saved=0";
         echo "<script>location.href='$URL'</script>";
